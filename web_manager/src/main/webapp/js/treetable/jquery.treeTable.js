...
(function ($) {
    ...
    $.fn.treeTable = function (opts) {
        ...
        function delChilds(pid) {
        	console.log(cMap);
            if (!pMap[pid]) { return false; }
            for (var i = 0; i < pMap[pid].length; i++) {
            	var childId = pMap[pid][i];
            	delChilds(childId);
            	if(childId !=null && childId !=undefined && childId!=0 ){
            		$('#' + childId, $treeTable).remove();
            	}
            }
            pMap[pid] = new Array();
        }
        ...
        $treeTable.setChilds = function(trsHtml) {
            var $trs = $(trsHtml);
 ...
 //           // 先删除旧的   再插入新的
  //          $("tr[pid="+pId+"]").remove();
            ...
            var insertId =   pMap[pId] && pMap[pId][pMap[pId].length - 1] || pId;
            if(pId <= 0) {
            	$($treeTable).find("tbody").after($trs);
            } else {
            	$('#' + insertId, $treeTable).after($trs);
            }
        ...
        $treeTable.refresh = function(trsHtml) {
           pMap = {}, cMap = {};
            var $trs = $treeTable.find('tr');
            initRelation($trs);    
        };
        $treeTable.delChilds = function(pid) {
        	delChilds(pid);
         };
         $treeTable.delNode = function(id) {
        	 delNode(id);
          }; 
