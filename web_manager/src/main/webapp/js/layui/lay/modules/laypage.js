...
layui.define(function(exports){
  ...
  Page.prototype.view = function(){
    ...
    return '<div class="layui-box layui-laypage layui-laypage-'+ (conf.skin ? (function(skin){
      ...
      return conf.skip 
      ? '<span class="layui-laypage-total">'+(conf.lang=='en'?' To ':' 到第')+'<input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" value="'+ conf.curr +'" class="layui-laypage-skip">'+(conf.lang=='en'?' Page ':' 页')
      + '<button type="button" class="layui-laypage-btn">'+(conf.lang=='en'?' Go ':' 确定')+'</button></span>' 
      : '';
    }() 
   /* +"<span><select><option>10</option><option>10</option><option>10</option></select></span>"*/
   + "<span style='border: ;font-weight: inherit;color: initial;line-height: 30px;'>"+ (conf.lang=='en'?' Total ':' 共') +"&nbsp;&nbsp;" + conf.total +"&nbsp;&nbsp;"+ (conf.lang=='en'?'Records ':'条') +"</span>" +
    '</div>'
    ;
  };
