...
<table align="center" border="1" cellspacing="0" width="680">
	...
	<tr>
		<th>密码</th>
		<th align="left">
			<input type="text" name="pass" value="${users.pass}" />
		</th>
		<th>年龄</th>
		<th align="left">
...
<table align="center" border="1" cellspacing="0" width="680">
	...
	<tr>
		...
		<td>${users.age}</td>
		<td width="120">
			<a href="editUsers.action?id=${users.id}">修改</a>&nbsp;
			<a href="deleteUsers.action?id=${users.id}">删除</a>
		</td>
	</tr>
	</c:forEach>
</table>
...
<table  align="center" border="0" cellspacing="0" width="680">
 ...
 --%>
  ...
  <form id="bottomPageForm" name="bottomPageForm" method="post" action="queryUsers.action">  
    <table class="pagebox" width="680" border="0" align="center" cellpadding="6" cellspacing="0"  style="margin-top:10px;">
      <tr>
        <td height="10"><script type="text/javascript" src="${ctx}/script/pager.js">;</script> 
          <script
			type="text/javascript">
            var pager = new Pager(document.bottomPageForm, ${pager.recordCount}, ${pager.pageSize}, ${pager.currentPage});
            pager.addHiddenInputs("users.id", "${users.id}");
            pager.addHiddenInputs("users.name", "${users.name}");
			pager.addHiddenInputs("users.pass", "${users.pass}");
            pager.addHiddenInputs("users.age", "${users.age}");
