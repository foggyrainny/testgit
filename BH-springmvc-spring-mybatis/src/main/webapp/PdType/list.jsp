...
<title>产品类别表</title>
    ...
    <body>
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td valign="top" bgcolor="#F7F8F9">
                    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                            ...
                            <td colspan="4">
                                <table>
                                    <tr>
                                        ...
                                        <td valign="bottom"><h3 style="letter-spacing:1px;">产品类别表</h3></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                            ...
                            <td width="200%">
                                <table width="190%">
                                    <tr>
                                        <td colspan="2">
                                        <!--查询表单  -->
                                            <form action="${ctx }/PdType/listPdType" method="post">
                                         <input type="button" onclick="location.href='addPdType'" value="添加" />    
                                      &nbsp; &nbsp; 类别名称：<input type="text" name="pd_name"  value="${pdType.pd_name }" />
									  &nbsp; &nbsp;产品类别 ： <input type="text" name="pd_type"  value="${pdType.pd_type }" />
                        ...
                        type="checkbox" class="icheck" name="check-all" id="check-all" value="${PdType.pd_type}"/></th>
          ...
          <tbody>
          <c:forEach var="pdType" items="${pdTypeList}" varStatus="vs">
          <tr>
            <td align="center">
            <input type="checkbox" class="icheck" name="ids" id="id-${pdType.pd_type}" value="${pdType.pd_type}" >
            </td>
           
            <td align="center">${pdType.pd_type}</td>
            <td align="center">${pdType.pd_name}</td>
            <td align="center">${pdType.order_sort}</td>
           
			<td align="center"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${pdType.add_date}" /></td>
			<td align="center">
				<input type="button"  onclick="location.href='deletePdType?pd_type=${pdType.pd_type}'" value="删除"/>
				<input type="button"  onclick="location.href='updatePdType?pd_type=${pdType.pd_type}'" value="修改"/>
			</td>         
          </tr>
      </c:forEach>
      </tbody>
    ...
    <form action="listPdType" method="post">
	<tr>
		<td colspan="10" align="center">
			共${pager.recordCount}条记录    每页显示${pager.pageSize}条    分${pager.pageCount}页/第${pager.currentPage}页
		   	 <a style="cursor:pointer;text-decoration: underline;" href="listPdType?requestPage=${pager.firstPage}">首页</a>
		   	 ...
		   	 <c:choose>
		   	 	...
		   	 	<c:otherwise>
		   	 		<a style="cursor:pointer;text-decoration: underline;" href="listPdType?requestPage=${pager.priviousPage}">上一页</a>
		   	 	</c:otherwise>
		   	 </c:choose>
		   	 ...
		   	 <c:choose>
		   	 	...
		   	 	<c:otherwise>
		   	 		<a style="cursor:pointer;text-decoration: underline;" href="listPdType?requestPage=${pager.nextPage}">下一页</a>
		   	 	</c:otherwise>
		   	 </c:choose>
		   	 
		   	 <a style="cursor:pointer;text-decoration: underline;" href="listPdType?requestPage=${pager.lastPage}">尾页</a>
